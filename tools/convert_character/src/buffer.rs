...
impl BufferBuilder {
    ...
    /// Append PNG-encoded image bytes. No target; image data lives in
    /// its own buffer view per the glTF spec.
    pub fn add_image_png(&mut self, bytes: &[u8]) -> Index<View> {
    ...
    /// Append untargeted data (inverse-bind matrices, animation samplers).
    pub fn add_untargeted(&mut self, bytes: &[u8]) -> Index<View> {
        self.add_view(bytes, None, None)
    }
