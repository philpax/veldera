...
#define_import_path bevy_pbr_atmosphere_planet::functions
...
//   to view space, but with the camera positioned on a local tangent plane
//   where Y points in the local "up" direction (radial from planet center).
//   This enables the non-linear latitude parametrization the paper uses
//   to concentrate detail near the horizon.


// CONSTANTS
const FRAC_PI: f32 = 0.3183098862; // 1 / pi
const FRAC_2_PI: f32 = 0.15915494309;  // 1 / (2*pi)
const FRAC_3_16_PI: f32 = 0.0596831036594607509; // 3 / (16*pi)
const FRAC_4_PI: f32 = 0.07957747154594767; // 1 / (4*pi)
const ROOT_2: f32 = 1.41421356; // sqrt(2)
...
fn multiscattering_lut_r_mu_to_uv(r: f32, mu: f32) -> vec2<f32> {
    let u = 0.5 + 0.5 * mu;
    let v = saturate((r - atmosphere.bottom_radius) / (atmosphere.top_radius - atmosphere.bottom_radius));
...
fn sky_view_lut_r_mu_azimuth_to_uv(r: f32, mu: f32, azimuth: f32) -> vec2<f32> {
    ...
    // l is latitude in [-pi/2, pi/2] and v is texture coordinate in [0,1].
...
fn sample_sky_view_lut(r: f32, ray_dir_as: vec3<f32>) -> vec3<f32> {
    // In atmosphere space, Y is up, so ray_dir_as.y is the cosine of the zenith angle.
    let mu = ray_dir_as.y;
...
fn sample_density_lut(r: f32, component: f32) -> vec3<f32> {
    // Return zero density outside the atmosphere bounds.
    if r > atmosphere.top_radius || r < atmosphere.bottom_radius {
        return vec3(0.0);
    }
...
fn sample_scattering_lut(r: f32, neg_LdotV: f32) -> vec3<f32> {
    // Return zero scattering outside the atmosphere bounds.
    if r > atmosphere.top_radius || r < atmosphere.bottom_radius {
        return vec3(0.0);
    }
    let normalized_altitude = (r - atmosphere.bottom_radius) / (atmosphere.top_radius - atmosphere.bottom_radius);
...
/// All positions and directions should be in atmosphere space (where Y is up).
fn sample_local_inscattering(local_scattering: vec3<f32>, ray_dir: vec3<f32>, atmo_pos: vec3<f32>) -> vec3<f32> {
...
/// Sample sun radiance for a given ray direction in atmosphere space.
fn sample_sun_radiance(ray_dir_as: vec3<f32>) -> vec3<f32> {
    let view_pos = get_view_position();
    let r = length(view_pos);
    let up = normalize(view_pos);
    // In atmosphere space, mu_view is the dot product of ray direction with up (both in atmo space).
...
///
/// For spherical planets, this uses the camera_radius uniform instead of deriving
/// the position from the view world position. The camera is placed at (0, r, 0)
/// in atmosphere space, where r is the distance from the planet center.
fn get_view_position() -> vec3<f32> {
    // Use the camera_radius uniform which is computed from the ECEF position.
    // In atmosphere space, the camera is at (0, r, 0) where Y is the local up direction.
    let r = atmosphere_transforms.camera_radius;
    var world_pos = vec3(0.0, r, 0.0);
    return clamp_to_surface(atmosphere, world_pos);
}

// For atmosphere-space calculations, the local "up" is now the Y axis (since we transform
// into a frame where local up = Y). This function computes the local up direction
// at a point along the ray.
